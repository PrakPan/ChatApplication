...
const socketHandler = (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    logger.info(
      `✅ User connected: ${socket.user.email} (${socket.id}) - UserId: ${userId}`
    );
    console.log(
      "📋 Connected users map updated. Total connected:",
      connectedUsers.size
    );
    ...
    socket.on("chat:join", async ({ recipientId }) => {
      try {
        ...
        if (recipientSocketId) {
          io.to(recipientSocketId).emit("messages:delivered", {
            conversationId,
          });
        }
      } catch (error) {
      ...
      try {
        ...
        if (socket.user.role === "host") {
          ...
          if (host && host.isOnline) {
            ...
            logger.info(
              `Host ${socket.user.email} disconnected, lastSeen updated`
            );
          }
        }
      ...
      if (inactiveHosts.length > 0) {
        for (const host of inactiveHosts) {
          ...
          if (isStillConnected) {
            ...
            console.log(
              `✅ Host ${host._id} is still connected, updated lastSeen`
            );
          } else {
        ...
        logger.info(
          `Cleanup: Processed ${inactiveHosts.length} potentially inactive hosts`
        );
      }
    } catch (error) {
